...
class NodeFormer(nn.Module):
    ...
    def add_adapter(self, adapter_class, positions, type):
        """
        ...
        Args:
            adapter_class (Union[nn.Module, str]): The adapter class to be added. If a string is provided, it should be the full
                                                path to the module.
            positions (str): Position to insert the adapter. Valid options are:
        ...
        for position in positions:
            if position == 'pre':
                for layer in range(self.num_layers):
                    self.pre_adapters[str(layer)] = instantiate(adapter_class, hidden_size=self.hidden_channels)
            elif position == 'post':
                for layer in range(self.num_layers):
                    self.post_adapters[str(layer)] = instantiate(adapter_class, hidden_size=self.hidden_channels)
    ...
    def forward(self, x, adjs, tau=1.0):
        ...
        for i, conv in enumerate(self.convs):
            ...
            if layer_str in self.pre_adapters:
                pre_adapter_output = self.pre_adapters[layer_str](layer_[i], adjs[0])
                if self.adapter_type == "sequential":
                    layer_[i] = pre_adapter_output

            if self.use_edge_loss:
                z, link_loss = conv(layer_[i], adjs, tau)
                link_loss_.append(link_loss)
            else:
                z = conv(layer_[i], adjs, tau)
